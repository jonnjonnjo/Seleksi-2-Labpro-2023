...
int ans(){
    int temp = 0;
    // just do hard code i gues
    if(oper[0] == '+' && oper[1] == '+' && oper[2] == '+') range(i,0,4) temp += arr[i];
    else if(oper[0] == '+' && oper[1] == '+' && oper[2] == '*') temp = (arr[0]+arr[1]+arr[2])*arr[3];
    else if(oper[0] == '+' && oper[1] == '*' && oper[2] == '+') temp = min(((arr[0]+arr[1])*arr[2] + arr[3]),arr[0]+ arr[1]*arr[2]+ arr[3]);
    else if(oper[0] == '*' && oper[1] == '+' && oper[2] == '+') temp = (arr[0]*arr[1] + arr[2] + arr[3]);
    else if(oper[0] == '+' && oper[1] == '*' && oper[2] == '*') temp = min(((arr[0]+arr[1])*arr[2]*arr[3]),(arr[0]+(arr[1])*arr[2]*arr[3]));
    else if(oper[0] == '*' && oper[1] == '*' && oper[2] == '+') temp = ((arr[0]*arr[1]*arr[2]) + arr[3]);
    else if(oper[0] == '*' && oper[1] == '+' && oper[2] == '*') temp = ((arr[0]*arr[1]) + arr[2])*arr[3];
    else temp = arr[0]*arr[1]*arr[2]*arr[3];
    return temp;
}

void solve(){
    range(i,0,4) cin >> arr[i];
   range(i,0,3) cin >> oper[i];
    int ra = INF;
    do{
        ra = min(ra,ans());
