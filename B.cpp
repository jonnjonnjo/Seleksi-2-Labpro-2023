...
void solve(){
    ...
    input2(tx,ty);
    int minim[maxn][maxn];
    range(i,0,maxn) range(j,0,maxn) minim[i][j] = -1;
    // start bfs from x,y
    queue<pair<pii,int>> q;
    q.push({{x,y},0});
    while(!q.empty()){
        pair<pii,int> node = q.front();
        minim[node.first.first][node.first.second] = node.second;
        q.pop();
        range(i,0,8){
            int newx = node.first.first + dx[i];
            int newy = node.first.second + dy[i];
            if(can(newx,newy) && minim[newx][newy] == -1){
                minim[newx][newy] = node.second+1;
