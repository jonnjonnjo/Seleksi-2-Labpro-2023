...
void solve(){
    map<int,int> num; // here we can map each player with its number
    vi v;
    v.pb(0); // for trash
    input2(n,q)
    range(i,1,n+1){
    ...
    // for each query, if we are asked to rotate it, just swap the player's number
    int length[q];
    range(i,0,q) cin >> length[i];
    range(i,0,q){
        inputarr(temparr,length[i]);
        if(length[i]%2 == 0){
            range(j,0,length[i]/2){
                int vall = num[v[temparr[j]]];
                int valr = num[v[temparr[length[i]-1-j]]];
                num[v[temparr[length[i]-1-j]]] = vall;
                num[v[temparr[j]]] = valr;
            }
        }else{
            ...
            vi nv;
            range(j,0,v.size()){
                if(j == temparr[(length[i]-1)/2]) continue;
                nv.pb(v[j]);
            }
            v = nv;
        }
    ...
    if(v.size() == 1) print("Tidak ada pemenang")
    else cout << v.size()-1 << ' ' << num[v[1]];
}
