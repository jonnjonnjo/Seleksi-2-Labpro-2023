...
void dfs(int idx, int par){
    ...
    timee++;
    timenetry[idx] = timee;
    lowestpossible[idx] = timee;
    for(int x : adj[idx]){
        if(x == par) continue;
        if(visited[x]){
            lowestpossible[idx] = min(lowestpossible[idx],timenetry[x]);
        }else{
            dfs(x,idx);
            lowestpossible[idx] = min(lowestpossible[idx],lowestpossible[x]);
            if(lowestpossible[x] > timenetry[idx]){
                cntallbridge++;
            }
        }
    }
