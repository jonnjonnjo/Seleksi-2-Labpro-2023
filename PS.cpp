...
void solve(){
    ...
    inputarr(arr,n)
    set<int> exist;
    range(i,0,n) exist.insert(arr[i]);
    // construct the prefum;
    range(i,1,n+1){
        range(j,1,(int)(sqrt(i))+1){
            if(i%j== 0){
                if(j != i/j && exist.find(j) != exist.end() && exist.find(i/j) != exist.end()){
                    prefsum[i]++;
                }
            }
        }
