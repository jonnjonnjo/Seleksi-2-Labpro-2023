...
int ans[maxn][1001] = {0};
int visited[maxn][1001] = {0};
int n;
int target;
int arr[maxn];

int dp(int price,int idx){
    if(price < 0) return INF;
    if(idx == n) return (price  == 0 ? 0 : INF);
    if(visited[price][idx]) return ans[price][idx];
    visited[price][idx] = 1;
    ans[price][idx] = min(dp(price-arr[idx],idx+1)+1,dp(price,idx+1));
    return ans[price][idx];
