...
int dp(int idx, int prev){
    if(prev < 0) return 0;
    if(idx == n) return prev == 0;
    if(visited[idx][prev]) return ans[idx][prev];
    int temp = 0; // there are two things that we can do. change the color or continue
    if(idx == 0){
        temp = (dp(idx+1,prev)*m)%MOD;
    }else{
        temp = (dp(idx+1,prev) + dp(idx+1,prev-1)*(m-1))%MOD;
    }
    visited[idx][prev] = 1;
    ans[idx][prev] = temp;
    return temp;
