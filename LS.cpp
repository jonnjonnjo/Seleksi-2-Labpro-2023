...
void solve(){
    ...
    range(i,0,n){
        int find = arr[i]+m;
        int r = (ub(arr,arr+n,find) - arr)-1; // do binary search
        saveans.pb({i,r});
    }
    int maks = 0;
    range(i,0,n) maks = max(maks,saveans[i].second-saveans[i].first+1);
    range2(i,n-1,0){
        if(saveans[i].second-saveans[i].first+1 == maks){
            int l = saveans[i].first;
            int r = saveans[i].second;
            cout << r-l + 1 << "\n";
            range(j,l,r+1){
                cout << arr[j] << ' ';
            }
